...
# The paramters could be one the following: 01, 02, 03, bottleneck, chicken, rimea1, rimea4, rimea6, rimea7
mode = -1
default = True
...
def state_machine():
    ## This function is used to change the state of pedestrians from this time step to the next by normal cost functions. ##
...
def state_dijmachine():
    ## This function is used to change the state of pedestrians from this time step to the next by Dijkstra algorithm. ##
    global field
    list = rp.Planner().open_list(map_grid)
    for element in list:
...
def circle_path():
    ## This function is used to calculate path specially for the cricular scenario (Task 3) ##
    list = rp.Planner().open_list(map_grid)
    for element in list:
...
class Application(tk.Tk):
    ...
    ## GUI design: Titles, buttons and displays
        global map_grid, field
        global target, start_list
        map_grid = numpy.loadtxt("./data/" + mode +".txt")
        ...
        label_time = tk.Label(self, text='Scenario name: '+ mode, font='Arial')
